...
import { createGame, disconnectGame, startGame, getBoard, getEnemyBoard, setTurn, finishGame, hasWon, takeTurn } from './game.ts';
...
function handleUserMessage(user: IUser, message: Message) {
  switch (message.type) {
    ...
    case MessageType.TAKE_TURN: {
      ...
      if(!userBoard.locked || !enemyBoard.locked) throw Error("Game board needs to be locked in");
      if(user.game.turn !== user.game.users.indexOf(user)) throw Error("Not your turn");
  ...
  socket.onmessage = (e) => {
    ...
    try {
      ...
      if (user) {
        ...
        if (message.ref) {
          sendMessage(socket, { type: MessageType.ACK, ref: message.ref, payload: response });
        }
      }
    } catch (e) {
      if (message.ref) {
        sendMessage(socket, { type: MessageType.ERROR, ref: message.ref, payload: e.message });
      }
    }
  };
